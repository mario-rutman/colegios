...
medio1_tur_a <- col_descartes_resul_2014 %>% 
  filter(Ensino == "1º Médio" & Turma == "a") %>%
...
# Gráfico com boxplots das médias de todas as matérias.
boxplot(col_descartes_resul_2014[,34:43]) # É bem lógico, escolhe a tabela de onde vai extrair os dados
# e dá o intervalo de colunas que interessa.
# Gráficos com os boxplots das notas finais de todas matérias.
boxplot(col_descartes_resul_2014[,64:73])
boxplot(medio1_tur_a$acimade) # Resultado esperado pois é uma distribuição de notas divididas por percentil.
...
seq2 <- seq(5, 10, length = 20) #20 números distribuídos entre 5 e 10.

# Criando gráfico vazio usando type = "n".
plot(medio1_tur_a$p1_port, medio1_tur_a$acimade, type = "n",
     xlab = "1ª prova de Portugês", ylab = "% de alunos com notas menores que a sua")
# Adicionar os nomes do alunos.
points(medio1_tur_a$p1_port, medio1_tur_a$acimade, 
       pch = as.character(medio1_tur_a$NOME))

# Criando gráficos Beeswarm.
beeswarm(medio1_tur_a$p1_port)# São os 31 alunos, cada um no seu centil.
...
plot(medio1_tur_a$p1_port, type = "l", xlab = "% de alunos acima de você", ylab = "nota", las = 1, col = "blue", lwd=4, cex.axis=0.8)

#Gráfico multiples lines.
# Criando o data frame df.
# Repetindo 1,2,3,4 e 5 9 vezes. Criando 45 números entre 1 e 100.
# Colando category a números de 1 a 9, de 5 em 5.
set.seed(45)
...
# Criando o multiples lines para o Colégio Descartes.
# Cada aluno será uma categoria e as p1, p2, p3... ficarão no eixo do x.
# Assim as 3 provas deverão ficar em uma mesma coluna e não em 3 como estão hoje. Ao trabalho.
...
# Fazer um gráfico com regressão linear de Português vs. Redação.
# Se fossem alunos de verdade esperar-se-ia uma correlação forte. Vejamos o que acontece.
...
# Usando o abline() para fazer a linha da regressão linear.
abline(linear_model, lty = 2)

# Colocar o nome do aluno que tirou determinada combinação de notas.
...
# Colocando o nome no gráfico colméia. 
beeswarm(medio1_tur_a$p1_port)
# Criando a regra de colocação do nome.
